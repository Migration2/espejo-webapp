...
import {
  ContactPointBikeModel,
  ContactPointStateModel,
  EstacionModel,
  StationKeepAliveModel,
  StationOperationTime,
  StatisticContactPoints,
} from "../../../models/estacion.model";
import {
  mantenimientoEstacionModel,
  finMantenimientoEstacionModel,
...
export class EstacionComponent implements OnInit {
  ...
  private stationKeepAliveData: StationKeepAliveModel =
    new StationKeepAliveModel();
  ...
  constructor(
    ...
    this.loadStationTransactionsData(
      this.idEstacion,
      this.fechaAnterior,
      this.fechaActual
    );
  ...
  private loadStationKeepAlive(stationCode: string) {
    this.estacionservice.getStationKeepAlive(stationCode).subscribe(
      (response) => {
        console.log(response);

        this.stationKeepAliveData = response;
        this.matchStationKeepAliveData(
          this.datosEstacion,
          this.stationKeepAliveData
        );
      },
  ...
  private matchStationKeepAliveData(
    stationData: EstacionModel,
    stationKeepAliveData: StationKeepAliveModel
  ) {
    let statusStation = this.getStatusStation(stationKeepAliveData);
    stationData.statusName = statusStation;
  }

  private getStatusStation(
    stationKeepAliveData: StationKeepAliveModel
  ): string {
    if (stationKeepAliveData.timeWithoutReport <= 5) {
      return this.IN_SERVICE_STATUS;
    } else if (
      stationKeepAliveData.timeWithoutReport > 5 &&
      stationKeepAliveData.timeWithoutReport <= 10
    ) {
  ...
  private loadContactPointData(contactPoints: Array<any>) {
    ...
    this.paginateContactPointsData(
      contactPoints,
      this.pageSize,
      this.pageNumber
    );
  ...
  ngOnInit() {
    this._createPoollingRefreshStationData(this.idEstacion);
  }

  private _createPoollingRefreshStationData(idEstacion: number) {
    if(!idEstacion){
        return;
    }
  ...
  private _createOptionsDatatableTransactions(
    bonotes: {
      extend: string;
      text: string;
      messageTop: string;
    ...
    return {
      columnDefs: [{ width: "50%", targets: 1 }],
      responsive: true,
      language: {
        url: CDN_SPANISH_JSON,
      },
  ...
  private _createButtonsDtOptions(
    bonotes: {
      extend: string;
      text: string;
      messageTop: string;
  ...
  private _createOptionsTatatableTransactions(
    bonotes: {
      extend: string;
      text: string;
      messageTop: string;
  ...
  private getStatisticsContactPoints(
    datosEstacion: EstacionModel
  ): StatisticContactPoints {
    ...
    let availableBikesPercentage: number = this.computePercentage(
      contactPoints,
      availableBikes
    );
    let looseContactPoints: number = contactPoints - availableBikes;
    let looseContactPointsPercentage: number = this.computePercentage(
      contactPoints,
      looseContactPoints
    );
    let contactPointsInMto: number =
      this.getCountBikesInMaintenance(puntosContacto);
    let contactPointsInMtoPercentage: number = this.computePercentage(
      contactPoints,
      contactPointsInMto
    );
    return new StatisticContactPoints(
      contactPoints,
      looseContactPoints,
      availableBikes,
      contactPointsInMto,
      availableBikesPercentage,
      looseContactPointsPercentage,
      contactPointsInMtoPercentage
    );
  }
  ...
  private loadStationOperationTime(stationCode: string) {
    this.estacionservice
      .getStationOperationTimeByCode(stationCode)
  ...
  private loadStationTransactionsData(
    idStation: string,
    initialDate: Date,
    endDate: Date
  ) {
    ...
    this.estacionservice
      .stationTransactions(idStation, initialDateString, endDateString)
      .subscribe((res) => {
  ...
  private loadStationMaintenanceData(stationId: string) {
    this.mantenimientoService
      .getManttosStation(stationId)
      .subscribe((responseMan) => {
  ...
  onSubmitFin() {
    this.finMantenimientoEstacionModel.id =
      this.mantenimientoHistorial[this.numeroMantenimientos].id;
    this.mantenimientoService.setManttoStationFin(
      this.finMantenimientoEstacionModel
    );
    this.router.navigate(["administrarEstaciones"]);
  }
  ...
  updateOperationData() {
    ...
    this.estacionservice.updateOperationTime(newStationOperationTime).subscribe(
      (response) => {
  ...
  recuperarHistorial(anterior, actual) {
    ...
    this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      this.estacionservice
        .stationTransactions(this.idEstacion, anterior, actual)
        .subscribe((res) => {
          this.transacciones = res;
          this.contarDatos(res);
          this.dtTriggerTransacciones.next();
    ...
    for (const key in transacciones) {
      if (transacciones.hasOwnProperty(key)) {
        ...
        for (const i in fecha) {
          if (
            transacciones[key].loanDate.toString().substring(0, 10) ===
            fecha[i]["date"]
          ) {
            bandera = true;
            fecha[i]["count"] += 1;
  ...
  enableStation() {
    this.estacionservice.enableStation(this.datosEstacion.code).subscribe(
      (response) => {
        if (response.status == 202) {
  ...
  disableStation() {
    this.estacionservice.disableStation(this.datosEstacion.code).subscribe(
      (response) => {
        if (response.status == 202) {
  ...
  private subscribeTopicsWebSocket(isPetitionlockStation: boolean) {
    this.stomp.startConnect().then(() => {
      ...
      this.subcriptionStationsKeepAlive = this.stomp.subscribe(
        "/topic/station",
        (stationsKeepAliveData) => {
          this.loadStationsKeepAliveData(
            stationsKeepAliveData,
            isPetitionlockStation
          );
        }
      );
    });
  ...
  private loadStationsKeepAliveData(
    stationsKeepAliveData: StationKeepAliveModel,
    isLockStation: boolean
  ) {
    if (isLockStation) {
  ...
  private paginateContactPointsData(
    stationsData: Array<any>,
    pageSize: number,
    pageNumber: number
  ) {
    this.contactPointsDataForView = this.paginator.transform(
      stationsData,
      pageSize,
      pageNumber
    );
    this.contactPointsDataSouce = new ContactPointsDataSource(
      this.contactPointsDataForView
    );
  }

  public selectedContactPoint: ContactPointStateModel =
    new ContactPointStateModel();
  ...
  putBike() {
    ...
    let contactPointBike = new ContactPointBikeModel(
      codeStation,
      idContactPoint,
      idBike
    );
  ...
  public openContactPoint() {
    ...
    this.estacionservice
      .openContactPoint(codeStation, contactPoint)
      .subscribe((res) => {
        if (res.status == 202) {
  ...
  public closeContactPoint() {
    ...
    let contactPoint: string = this.selectedContactPoint.alias;
    this.estacionservice
      .closeContactPoint(codeStation, contactPoint)
      .subscribe((res) => {
        if (res.status == 202) {
